...
!=====================================================================
  ...
  ! seismograms
  use specfem_par, only: seismograms_d,seismograms_v,seismograms_a,seismograms_p,displ_record,veloc_record,accel_record
  ! wavefields
  ...
  double precision :: dvxdx,dvxdy,dvxdz,dvydx,dvydy,dvydz,dvzdx,dvzdy,dvzdz
!   double precision,dimension(3,3) :: Vmat,Rotmat
  ...
  displ_record(:,:,seismo_current) = displ(:,:)
  veloc_record(:,:,seismo_current) = veloc(:,:)
  accel_record(:,:,seismo_current) = accel(:,:)

! openmp solver
!$OMP PARALLEL if (num_elements > 100) &
!$OMP DEFAULT(NONE) &
!$OMP SHARED( &
!$OMP CUSTOM_REAL,NGLLX,NGLLY,NGLLZ,NDIM,ZERO, &
!$OMP SIMULATION_TYPE,NGLOB_AB,NSPEC_AB,ibool,NGLOB_ADJOINT, &
!$OMP seismo_current, &
!$OMP ispec_selected_source,ispec_selected_rec, &
!$OMP number_receiver_global,nrec_local, &
!$OMP nu_source,nu_rec, &
!$OMP hxir_store,hetar_store,hgammar_store, &
!$OMP USE_TRICK_FOR_BETTER_PRESSURE, &
!$OMP SAVE_SEISMOGRAMS_DISPLACEMENT,SAVE_SEISMOGRAMS_VELOCITY, &
!$OMP SAVE_SEISMOGRAMS_ACCELERATION,SAVE_SEISMOGRAMS_PRESSURE, &
!$OMP seismograms_d,seismograms_v,seismograms_a,seismograms_p, &
!$OMP ispec_is_acoustic,potential_acoustic,potential_dot_acoustic,potential_dot_dot_acoustic, &
!$OMP b_potential_acoustic,b_potential_dot_acoustic,b_potential_dot_dot_acoustic, &
!$OMP ispec_is_elastic,displ,veloc,accel,b_displ,b_veloc,b_accel,&
!$OMP ispec_is_poroelastic,displs_poroelastic,velocs_poroelastic,accels_poroelastic &
!$OMP ) &
!$OMP PRIVATE( &
!$OMP displ_element,veloc_element,accel_element, &
!$OMP dxd,dyd,dzd,vxd,vyd,vzd,axd,ayd,azd,pd, &
!$OMP dvxdx,dvxdy,dvxdz,dvydx,dvydy,dvydz,dvzdx,dvzdy,dvzdz, &
!$OMP rotation_seismo,hxir,hetar,hgammar,irec_local,irec,ispec,i,j,k,l) &
!$OMP FIRSTPRIVATE()
  ...
  do irec_local = 1,nrec_local
    ...
    case (1,2)
      ...
      if (ispec_is_elastic(ispec)) then
        call compute_interpolated_partial_v_partial_x(veloc,NGLOB_AB, &
