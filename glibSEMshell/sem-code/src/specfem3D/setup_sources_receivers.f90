...
!=====================================================================
  ...
  write(*,*) 'setup_point_search_arrays'
  ! setup for point search
  call setup_point_search_arrays()

  write(*,*) 'setup_timesteps'
...
!
  ...
  logical,dimension(:),allocatable :: glob_is_used, spec_is_used ! add by Chang Guo
  character(len=MAX_STRING_LEN) :: filename
  integer :: fileid
...
! add by Guo Chang
  if (nrec > 2) then
    ! find used spec
    allocate(spec_is_used(NSPEC_AB),stat=ier)
    do i=1,NSPEC_AB
        spec_is_used(i) = .false.
    enddo
    do irec_local = 1,nrec_local
        irec = number_receiver_global(irec_local)
        ispec = ispec_selected_rec(irec)
    ...
    do ispec = 1,NSPEC_AB
        if (spec_is_used(ispec)) then
            NUsedSpec = NUsedSpec + 1
        endif
    enddo
    allocate(used_specid(NUsedSpec),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 8886')
    i = 0
    do ispec = 1,NSPEC_AB
        if (spec_is_used(ispec)) then
            i = i+1
    ...
    enddo
    deallocate(spec_is_used)
    ! find used glob
    allocate(glob_is_used(NGLOB_AB),stat=ier)
    do i=1,NGLOB_AB
        glob_is_used(i) = .false.
    enddo
    do l = 1,NUsedSpec
        ispec = used_specid(l)
        do i=1,NGLLX
    ...
    do iglob = 1,NGLOB_AB
        if (glob_is_used(iglob)) then
            NUsedGlob = NUsedGlob + 1
        endif
    enddo
    allocate(used_globid(NUsedGlob),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 8887')
    i = 0;
    do iglob = 1,NGLOB_AB
        if (glob_is_used(iglob)) then
            i = i + 1
    ...
    enddo
    deallocate(glob_is_used)
  else
    fileid = myrank+300
    write(filename, "('/proc',i6.6,'specinfo.bin')") myrank
    open(unit=fileid,file=trim(OUTPUT_FILES)//filename,status='old', &
                form='unformatted',action='read',access='stream')
    read(fileid) NUsedGlob
    allocate(used_globid(NUsedGlob),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 8887')
    read(fileid) used_globid
    close(fileid)
  endif

  ! record wavefield
!   allocate(displ_record(NDIM,NUsedGlob,nlength_seismogram),stat=ier)
!   if (ier /= 0) call exit_MPI_without_rank('error allocating array 8888')
!   displ_record(:,:,:) = 0._CUSTOM_REAL
  allocate(veloc_record(NDIM,NUsedGlob),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 8889')
  veloc_record(:,:) = 0._CUSTOM_REAL
!   allocate(accel_record(NDIM,NUsedGlob,nlength_seismogram),stat=ier)
!   if (ier /= 0) call exit_MPI_without_rank('error allocating array 8890')
!   accel_record(:,:,:) = 0._CUSTOM_REAL
...
!
  ...
  do irec = 1,nrec
    ...
    if (islice_selected_rec(irec) /= 0) then
      if (myrank == 0) then
        ! write(*,*) myrank, ''
        ! write(*,*) myrank, 'nrec: ', nrec
        ! write(*,*) myrank, 'irec: ', irec
        ! write(*,*) myrank, 'islice_selected_rec(irec): ',islice_selected_rec(irec)
        ! write(*,*) myrank, 'recv xelm: ', xelm
        call recvv_cr(xelm,NGNOD,islice_selected_rec(irec),0)
        ! write(*,*) myrank, 'recv xelm: ', xelm
        ! write(*,*) myrank, 'recv yelm: ', yelm
        call recvv_cr(yelm,NGNOD,islice_selected_rec(irec),0)
        ! write(*,*) myrank, 'recv yelm: ', yelm
        ! write(*,*) myrank, 'recv zelm: ', zelm
        call recvv_cr(zelm,NGNOD,islice_selected_rec(irec),0)
        ! write(*,*) myrank, 'recv zelm: ', zelm
      else if (myrank == islice_selected_rec(irec)) then
        ! write(*,*) myrank, 'send xelm: ', xelm
        call sendv_cr(xelm,NGNOD,0,0)
        ! write(*,*) myrank, 'send xelm: ', xelm
        ! write(*,*) myrank, 'send yelm: ', yelm
        call sendv_cr(yelm,NGNOD,0,0)
        ! write(*,*) myrank, 'send yelm: ', yelm
        ! write(*,*) myrank, 'send zelm: ', zelm
        call sendv_cr(zelm,NGNOD,0,0)
        ! write(*,*) myrank, 'send zelm: ', zelm
      endif
    endif
