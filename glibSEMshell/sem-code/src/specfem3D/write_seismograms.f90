...
! writes the seismograms with time shift
  ...
  if (mod(it-1,NTSTEP_BETWEEN_OUTPUT_SAMPLE) == 0) then
    if (it == 1) then
        IO_OUTPUT = myrank + IOUT + 100
        write(outputname, "('/proc',i6.6,'displ.bin')") myrank
        open(unit=IO_OUTPUT,file=trim(OUTPUT_FILES)//outputname,status='unknown', &
                    form='unformatted',action='write',access='stream')
        write(IO_OUTPUT) NDIM,nlength_seismogram,NUsedGlob
        seismo_current = 0
    endif
    do k = 1,NUsedGlob
        j = used_globid(k)
        do i = 1,NDIM
            veloc_record(i,k) = displ(i,j)
        enddo
    enddo
    write(IO_OUTPUT) veloc_record
    seismo_current = seismo_current + 1
    if (seismo_current == nlength_seismogram) then
        close(IO_OUTPUT)
    endif
  endif
  call synchronize_all()
  return
  write(*,*) 'This should not appear. info from write_seismograms.f90:79'
  ...
  return
  write(*,*) 'This should not appear. info from write_seismograms.f90'
...
! write seismograms to text files
  ...
  if (ASDF_FORMAT) then
    ...
    ! nothing left to do (outputs only single ASDF seismogram file)
