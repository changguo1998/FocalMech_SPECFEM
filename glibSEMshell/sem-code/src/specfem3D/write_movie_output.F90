  ...
  if (ELASTIC_SIMULATION .or. POROELASTIC_SIMULATION) then
    ...
    if (ELASTIC_SIMULATION) then
      ...
      call wmo_movie_div_curl(veloc, &
                              ...
                              ispec_is_elastic)
    !   call wmo_movie_spacial_partial(veloc, &
    !                           dvxdxl,dvxdyl,dvxdzl,dvydxl,dvydyl, &
    !                           dvydzl,dvzdxl,dvzdyl,dvzdzl,ispec_is_elastic)
    ...
    ! write(outputname,"('/proc',i6.6,'_dvxdx_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvxdx'
    ! write(IOUT) dvxdxl
    ! close(IOUT)

    ! write(outputname,"('/proc',i6.6,'_dvxdy_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvxdy'
    ! write(IOUT) dvxdyl
    ! close(IOUT)

    ! write(outputname,"('/proc',i6.6,'_dvxdz_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvxdz'
    ! write(IOUT) dvxdzl
    ! close(IOUT)

    ! write(outputname,"('/proc',i6.6,'_dvydx_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvydx'
    ! write(IOUT) dvydxl
    ! close(IOUT)

    ! write(outputname,"('/proc',i6.6,'_dvydy_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvydy'
    ! write(IOUT) dvydyl
    ! close(IOUT)

    ! write(outputname,"('/proc',i6.6,'_dvydz_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvydz'
    ! write(IOUT) dvydzl
    ! close(IOUT)

    ! write(outputname,"('/proc',i6.6,'_dvzdx_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvzdx'
    ! write(IOUT) dvzdxl
    ! close(IOUT)

    ! write(outputname,"('/proc',i6.6,'_dvzdy_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvzdy'
    ! write(IOUT) dvzdyl
    ! close(IOUT)

    ! write(outputname,"('/proc',i6.6,'_dvzdz_it',i6.6,'.bin')") myrank,it
    ! open(unit=IOUT,file=trim(LOCAL_PATH)//trim(outputname),status='unknown',form='unformatted',iostat=ier)
    ! if (ier /= 0) stop 'error opening file dvzdz'
    ! write(IOUT) dvzdzl
    ! close(IOUT)
  endif
...
!----------------------------------
  subroutine wmo_movie_spacial_partial(veloc, &
    dvxdxl,dvxdyl,dvxdzl,dvydxl,dvydyl, &
    dvydzl,dvzdxl,dvzdyl,dvzdzl,ispec_is)

! calculates special partial
! add by changguo
...
! real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB),intent(out) :: div, curl_x, curl_y, curl_z
! real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB),intent(inout) :: velocity_x,velocity_y,velocity_z
real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB),intent(out):: dvxdxl,dvxdyl, &
dvxdzl,dvydxl,dvydyl,dvydzl,dvzdxl,dvzdyl,dvzdzl
logical,dimension(NSPEC_AB),intent(in) :: ispec_is
...
dvxdxl(i,j,k,ispec) = xixl*tempx1l + etaxl*tempx2l + gammaxl*tempx3l
dvxdyl(i,j,k,ispec) = xiyl*tempx1l + etayl*tempx2l + gammayl*tempx3l
dvxdzl(i,j,k,ispec) = xizl*tempx1l + etazl*tempx2l + gammazl*tempx3l

dvydxl(i,j,k,ispec) = xixl*tempy1l + etaxl*tempy2l + gammaxl*tempy3l
dvydyl(i,j,k,ispec) = xiyl*tempy1l + etayl*tempy2l + gammayl*tempy3l
dvydzl(i,j,k,ispec) = xizl*tempy1l + etazl*tempy2l + gammazl*tempy3l

dvzdxl(i,j,k,ispec) = xixl*tempz1l + etaxl*tempz2l + gammaxl*tempz3l
dvzdyl(i,j,k,ispec) = xiyl*tempz1l + etayl*tempz2l + gammayl*tempz3l
dvzdzl(i,j,k,ispec) = xizl*tempz1l + etazl*tempz2l + gammazl*tempz3l

else ! regular element

dvxdxl(i,j,k,ispec) = xix_regular*tempx1l
dvxdyl(i,j,k,ispec) = xix_regular*tempx2l
dvxdzl(i,j,k,ispec) = xix_regular*tempx3l

dvydxl(i,j,k,ispec) = xix_regular*tempy1l
dvydyl(i,j,k,ispec) = xix_regular*tempy2l
dvydzl(i,j,k,ispec) = xix_regular*tempy3l

dvzdxl(i,j,k,ispec) = xix_regular*tempz1l
dvzdyl(i,j,k,ispec) = xix_regular*tempz2l
dvzdzl(i,j,k,ispec) = xix_regular*tempz3l
...
end subroutine wmo_movie_spacial_partial
