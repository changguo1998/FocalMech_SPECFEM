  ...
  do it = 1, num_node
    ...
    if (ADIOS_FOR_MESH) then
      ...
      open(unit=27,file=prname_lp(1:len_trim(prname_lp))//'external_mesh.bin', &
            status='old',access='stream',action='read',form='unformatted',iostat=ier)
      read(27) NSPEC_AB
      read(27) NGLOB_AB
    ...
    if (ADIOS_FOR_MESH) then
      ...
      open(unit = 28,file = trim(local_data_file),status='old', &
            action='read',form ='unformatted',access='stream',iostat=ier)
      if (ier /= 0) then
...
! writes element information
  ...
  do it = 1, num_node
    ...
    if (ADIOS_FOR_MESH) then
      ...
      ! gets number of elements and global points for this partition
      open(unit=27,file=prname_lp(1:len_trim(prname_lp))//'external_mesh.bin', &
            status='old',access='stream',action='read',form='unformatted')
      read(27) NSPEC_AB
      read(27) NGLOB_AB
      read(27) NSPEC_IRREGULAR
    endif
  ...
  if (filename(1:8) == 'dvxdx_it' .or. filename(1:8) == 'dvxdy_it' .or. filename(1:8) == 'dvxdz_it' .or. &
      filename(1:8) == 'dvydx_it' .or. filename(1:8) == 'dvydy_it' .or. filename(1:8) == 'dvydz_it' .or. &
      filename(1:8) == 'dvzdx_it' .or. filename(1:8) == 'dvzdy_it' .or. filename(1:8) == 'dvzdz_it') then
    data_array_name = trim(filename(1:5)) ! "curl_X",..
  endif
