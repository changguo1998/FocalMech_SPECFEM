    ...
    tasks[i] = @task begin
        while true
            ...
            try
                ...
                if isfile(joinpath(PTH["dat_selectstation"], e, "auto.jld2"))
                    if !isfile(joinpath(PTH["dat_selectstation"], e, "result", "result_stage3.jld2"))
                        mkpath(joinpath(PTH["dat_selectstation"], e, "result"))
                        touch(joinpath(PTH["dat_selectstation"], e, "result", "begin.txt"))
                        run(Cmd(Cmd(["julia", "-t", "8", "inverse.jl", e]); dir=PTH["invscript"]); wait=true)
                    end
                end
            catch x
                lock(lk) do
